...
The strict equality (===) operator checks whether its two operands are equal and returning a Boolean result.and it's also return true if both the value and type of operands are the same. Since type conversion is not done, so even if the value stored in operands is the same but their type is different the operation will return false.
